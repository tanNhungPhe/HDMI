...
void SendDSIcfg(void)
{
	...
	/* Panel Init */
	/* vậy thì có cần thiết để cho các màn hình khác không*/
	TC358870_i2c_Write(0x0504,0x0015, 2);
	TC358870_i2c_Write(0x0504,0x00ff, 2);

	TC358870_i2c_Write(0x0504,0x0015, 2);
	TC358870_i2c_Write(0x0504,0x01fb, 2);
	...
	TC358870_i2c_Write(0x0504,0x0015, 2);
	TC358870_i2c_Write(0x0504,0x08d3, 2);

	TC358870_i2c_Write(0x0504,0x0015, 2);
	TC358870_i2c_Write(0x0504,0x0ed4, 2);

	TC358870_i2c_Write(0x0504,0x0005, 2); // DCSCMD_Q
	TC358870_i2c_Write(0x0504,0x0011, 2); // DCSCMD_Q
	HAL_Delay(120);

	/* Additional LQ055 trial init sequence (try multiple encodings) */
	printf("Attempting LQ055 init sequence: set pixel format and exit sleep...\r\n");

	/* Try sending Set Pixel Format (0x3A, param 0x77 => 24-bit) in normal write */
	TC358870_i2c_Write(0x0504, 0x3A77, 2);
	HAL_Delay(5);

	/* Also try sending the same as Big-Endian (helper) in case chip expects different order */
	TC358870_I2C_Write_BigIn(0x0504, 0x3A77, 2);
	HAL_Delay(5);

	/* Enable Tear Effect (0x35) with mode 0 */
	TC358870_i2c_Write(0x0504, 0x3500, 2);
	HAL_Delay(5);
	TC358870_I2C_Write_BigIn(0x0504, 0x3500, 2);
	HAL_Delay(5);

	/* Sleep Out (0x11) and Display ON (0x29) */
	TC358870_i2c_Write(0x0504, 0x0011, 2); // Sleep Out
	HAL_Delay(120);
	TC358870_i2c_Write(0x0504, 0x0029, 2); // Display ON
	HAL_Delay(100);
	...
	printf("LQ055 init sequence attempted.\r\n");
}
